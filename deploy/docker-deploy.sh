#!/bin/bash
# Docker Deployment Script for AgentsFlowAI
# Usage: ./deploy/docker-deploy.sh
# This script deploys the application using Docker containers instead of PM2
...
test_ssh_connection() {
    ...
    if [ "$USE_PASSWORD_AUTH" = true ]; then
        ...
        # Set password
        export SSHPASS="Uk8&-47Oq-Qp#R3n;Ks3"
        set +e
...
# 2. Sync files to server (excluding unnecessary files)
...
if [ "$USE_PASSWORD_AUTH" = false ]; then
    rsync -avz --delete \
      ...
      --exclude='.env.local' \
      --exclude='.env.example' \
      --exclude='deploy/docker-deploy.sh' \
...
# 3. Deploy with Docker on server
...
if ! command -v docker &> /dev/null; then
    echo "❌ Docker not found. Please run setup-server.sh first to install Docker."
    exit 1
fi

if ! command -v docker-compose &> /dev/null; then
    echo "❌ Docker Compose not found. Please run setup-server.sh first to install Docker Compose."
    exit 1
fi
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.production.template ]; then
        ...
        echo "⚠️  Please edit .env with your production values before running again"
        exit 1
    else
        echo "❌ Neither .env nor .env.production.template found"
        exit 1
    fi
fi

# Run database migrations (if using external database)
echo "🗄️  Running database migrations..."
if command -v npx &> /dev/null; then
    npx prisma migrate deploy 2>/dev/null || echo "No migrations needed or Prisma not available"
    npx prisma db seed 2>/dev/null || echo "Seeding skipped"
else
    echo "⚠️  npx not available, skipping database migrations"
fi
...
docker-compose -f docker-compose.prod.yml pull || echo "No images to pull or pull failed"
...
echo "App URL: https://agentsflowai.cloud"
echo ""
echo "Next steps:"
echo "- Check container status: ssh -i ~/.ssh/deploy_key deploy@srv1187860.hstgr.cloud \"cd /home/deploy/agentsflow-ai && docker-compose -f docker-compose.prod.yml ps\""
echo "- View logs: ssh -i ~/.ssh/deploy_key deploy@srv1187860.hstgr.cloud \"cd /home/deploy/agentsflow-ai && docker-compose -f docker-compose.prod.yml logs -f\""
